...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (inputs, labels, path) in enumerate(self.data_loader):
            # debugging
            ...
            if batch_idx % self.log_step == 0:
                self.logger.debug('Train Epoch: {} {} Loss: {:.6f}'.format(
                    ...
                    loss.item()))
                self.writer.add_image('face', make_grid(face.cuda(), nrow=4, normalize=True))
                self.writer.add_image('context', make_grid(context.cuda(), nrow=2, normalize=True))
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, labels, path) in enumerate(self.valid_data_loader):
                face, context = inputs['face'].to(self.device), inputs['context'].to(self.device)
                labels = labels.to(self.device)
